...
{
    public class ThirdPersonCharacter_Mod : MonoBehaviour
    {
		...
		[HideInInspector] public float burstInput;
        ...
        public void Move(Vector3 move, bool crouch, bool jump, Vector3 lookPos)
        {
            ...
            if (move.magnitude > 1 && burstInput==1.0f) move.Normalize();
			...
			//Move is a burst jump
			if (burstInput > 1.0f) {
				Vector3 extraGravityForce = (Physics.gravity*3) - Physics.gravity;
				rigidbody.AddForce(extraGravityForce);
				rigidbody.velocity = (transform.up + transform.forward) * burstInput;

			} else if (burstInput == -1.0f) {
			//Move is downward motion of a burst jump
				//The amount of downward force applied to player depends on her distance from the ground 
				float heightFromGround = transform.position.y;
				Vector3 extraGravityForce = (Physics.gravity*10*heightFromGround) - Physics.gravity;
				rigidbody.AddForce(extraGravityForce);
				rigidbody.velocity = ((5.0f* transform.up) + (-30.0f*transform.forward)) * burstInput;
				Debug.Log ("Fast forwarding down");
				//Reset burst input
				burstInput = 1.0f;
			} else {
